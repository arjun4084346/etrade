...
import static com.etrade.exampleapp.v1.terminal.ETClientApp.lineSeparator;
...
public class OrderClient extends Client {
	...
	public void parseResponse(final String response)throws Exception {
		...
		if (jsonObject.get("OrdersResponse")!= null) {
			...
			String titleFormat =
					"%10s %25s %25s %13s %10s %10s %15s %25s %15s %25s %15s" + System.lineSeparator() + System.lineSeparator();
			...
			while (orderItr.hasNext()) {
				...
				for (Object o : orderInstArr) {
					...
					responseData[8] =
							OrderUtil.getPrice(PriceType.getPriceType(String.valueOf(orderDetail.get("priceType"))), orderDetail);
					formatString.append("%15s");
					...
					fmt.format(formatString.toString(), responseData[0], responseData[1], responseData[2], responseData[3],
							responseData[4], responseData[5], responseData[6], responseData[7], responseData[8], responseData[9],
							responseData[10]);
